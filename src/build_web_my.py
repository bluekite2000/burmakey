...
/* the confirmation sits ON the bubble — placed below it, it slid under the
   next bubble or the suggestion strip and read as a background glitch */
.bub.copied::after{content:"✓ ကူးပြီး · copied";position:absolute;
left:50%;top:50%;transform:translate(-50%,-50%);
background:rgba(6,20,16,.92);color:#7ce3c3;border:1px solid #1c7963;
border-radius:9px;padding:5px 11px;font-size:12px;white-space:nowrap;z-index:3}
...
function commit(i,raw,pos,zeroKey){
  ...
  /* during the guided test, the typed Burglish IS the datum the test exists
     to collect — its consent covers exactly this */
  if(convo.mode==="test")convo.typed.push([t0,i!==null?LO[i]:raw,i!==null?pos:-1]);
  if(i!==null){
...
function msgText(){
  ...
  for(let i=0;i<words.length;i++){
    ...
    if(i>0){
      ...
      else if(pv.punct&&!w.punct&&!pv.space)s+=" ";   // standard Burmese spaces after ။ and ၊
    }
    s+=w.my;
  }
  return s;
...
function drawMsg(){
  ...
  if(!msgs.length&&!words.length){
    ...
    e.innerHTML="<b>သင့်စာသား ဒီမှာပေါ်မယ်</b>your message appears here — "+
      ...
      "<br><br>Type <b>help</b> for the keys · <b>test</b> for the guided test.";
    t.appendChild(e);
  }else{
...
function sendMsg(){
  ...
  if(!convo.mode&&TESTWORDS.includes(t.trim().toLowerCase())){
    words=[];prev=null;drawMsg();render();testOffer();
    $("inp").focus({preventScroll:true});return;
  }
...
/* Item sets: 3 anchors identical for every tester (clean between-user
   comparison), 7 copy items sampled per session from a 60-sentence pool
   chosen for syllable coverage (54 -> 338 syllables), then the fixed stress
   and free items. Each row logs its own target, so analysis needs no session
   metadata to know what was asked. */
const EX_ANCHOR = [{"k": "copy", "t": "ပြဿနာရှိပါသလား"}, {"k": "copy", "t": "ဘန်ကောက်မှာဘယ်အချိန်ရှိပြီလဲ"}, {"k": "copy", "t": "ဟုတ်ကဲ့ဒီမှာပါ"}];
const EX_POOL = ["နိုင်ငံခြားပညာသင်သွားရန်စိတ်အားထက်သန်သည်", "ကြက်သွန်မြိတ်ပဲပင်ပေါက်ကြက်ဥခေါက်ကမာကောင်ကြော်", "ဒါပေမဲ့ကျွန်တော်အနားယူဖို့လိုနေတယ်", "အကူအညီရရင်ကျေးဇူးတင်တာသဘာဝပဲ", "ခင်ဗျားသေသေချာချာစဉ်းစားပြီးမှလုပ်နော်", "ဤတွင်အပြန်အလှန်စကားပြောဆိုကြသည်", "ရေခဲပြင်မှာချော်လဲပြီးဖင်ထိုင်လျက်ကျတယ်", "အနုပညာလက်ရာကိုလေလံတင်ရောင်းချတယ်", "အခုတလောလောကကြီးမှာအယူသည်းသူတွေများတယ်", "တစ်ခေတ်နဲ့တစ်ခေတ်မတူတော့ဘူး", "ဒေါသဖြစ်လို့မျက်နှာနီလာသည်", "ပြည်တွင်းဖုန်းခအရမ်းသက်သာတယ်", "လန်းဆန်းသွားအောင်ကော်ဖီသောက်မယ်", "နို့နံ့သင်းတဲ့ဖရုံစေ့", "အဆောင်ပိုင်ရှင်အဒေါ်ကြီးကသဘောကောင်းလား", "နှစ်ယောက်စာစားပွဲတစ်ဝိုင်းလိုချင်တယ်", "မုတ်ဆိတ်မွေးတဆိတ်လောက်တိပေးပါ", "နံရံမှာပန်းချီချိတ်ထားတယ်", "သူ့ကိုလျှို့ဝှက်စွာတွေ့ဆုံတယ်", "ပိုက်ဆံအိတ်ယူဖို့ဖျက်ခနဲမေ့သွားတယ်", "ငရုတ်သီးခြောက်ကသားဆိုတော့ကျပ်ပါ", "ပြင်သစ်ရုရှားနဲ့အီတလီပါ", "အခက်ခဲပေါင်းစုံကိုဖြတ်သန်းသည်", "ဒီအရှုပ်အထွေးထဲကထွက်နိုင်ပါ့မလား", "အားလုံးကိုနိုင်ငံတကာစံအတိုင်းထုတ်လုပ်မယ်", "အနှံဘီယာပျော့လေးပဲပေးပါ", "အဝေးမှမိုးခြိမ်းသံကြားရသည်", "ညနေခင်းလက်ဖက်ရည်အချိန်ရောရှိလား", "စည်းကမ်းကိုလျစ်လျူရှုသည်", "အိမ်စာများကိုတွက်ချက်ဖြေရှင်းသည်", "အစီအစဉ်တွေကောသိလား", "ခင်ဗျားမိဘကိုသိပ်ရိုသေတာပဲ", "ဘူတာအထိလိုက်ပို့ပေးပါ", "ပဲဟင်းနဲ့အာလူးထောင်းကြော်", "ကျေးဇူတင်ပါတယ်ငြိမ်းငြိမ်းချမ်းချမ်းနေရစ်ပါ", "စားနေတုန်းဂရုစိုက်မှပေါ့", "မင်းငါ့အပေါ်မမျှတဘူး", "ဦးကြီးကျေးဇူးပြုပြီးမေးပါရစေ", "ခရီးလမ်းကြောင်းဘယ်လောက်ရှည်လဲ", "ဒါကကျွန်တော်တို့ရဲ့စာနာတတ်မှုပါ", "အမျိုးသမီးစားပွဲထိုးခေါင်းဆောင်", "ဦးညွတ်၍တလေးတစားတောင်းဆိုသည်", "အထုပ်အပိုးအပိုကြေးလား", "ဒီနေ့ရာသီဥတုအလွန်ကောင်းပါတယ်", "ဒါကလက်ခံစာရွက်နဲ့ပြန်အမ်းငွေပါ", "မွန်းလွဲနာရီရောက်မယ်", "ကောက်ရိုးဖျာခင်းပြီးစပါးလှန်းထားသည်", "ဒါကအသစ်ဆုံးနောက်ဆုံးမော်ဒယ်ပါ", "အဲဒါကိုအစပ်နည်းနည်းလျှော့ပေးပါ", "ကျွန်တော်အဖြူရောင်ကြိုက်တယ်", "လေဆိပ်ကိုယာဉ်စီးခဘယ်လောက်လဲ", "လက်မှတ်တန်ဖိုးကဘယ်လောက်လဲ", "ဒီနှစ်သူအမြဲကိုယ်ပူတယ်", "ခုနကကုန်တိုက်ကိုသွားပြီးပြီ", "သစ်တောထဲမှာယောင်လည်လည်ဖြစ်သည်", "ဆရာထပ်စစ်ပေးနိုင်မလား", "ဇွဲမာန်တင်းပြီးအလုပ်လုပ်သည်", "ပဟေဠိဖွက်သည်", "မကြာခဏအအေးမိတယ်", "အမြင်များဝိရောဓိဖြစ်နေသည်"];
const EX_STRESS = [{"k": "stress", "t": "ခင်ဗျားဘာအားကစားကစားလဲ", "n": "ရှည်သောခလုတ် · long-press glyphs"}, {"k": "stress", "t": "ဘာဖြစ်လို့လဲ", "n": "ရှည်သောခလုတ် · long-press glyphs"}, {"k": "stress", "t": "ဈေးက ၂၅၀၀ ကျပ်ပါ။", "n": "ဂဏန်းနှင့်ပုဒ်ကြီး · numbers and ။"}, {"k": "stress", "t": "ok ကျေးဇူးတင်ပါတယ်", "n": "အင်္ဂလိပ်စာ ရောသုံး · mixed English"}, {"k": "stress", "t": "မောင်မောင် ဘယ်မှာလဲ", "n": "နာမည် · a name, tests the unknown-word fallback"}];
const EX_FREE = {"k": "free", "t": "", "n": "သူငယ်ချင်းကို စာတစ်စောင်ရေးပါ · write a friend a message, anything you like"};
let EX_ITEMS = [];
function assembleItems(){
  const picks=[...EX_POOL];
  for(let i=picks.length-1;i>0;i--){          // Fisher-Yates
    const j=Math.floor(Math.random()*(i+1));
    [picks[i],picks[j]]=[picks[j],picks[i]];
  }
  EX_ITEMS = EX_ANCHOR
    .concat(picks.slice(0,7).map(t=>({k:"copy",t:t})))
    .concat(EX_STRESS)
...
/* ================= the conversation ================= */
const convo={mode:null,i:0,rows:[],answers:[],snap:null,t0:0,typed:[]};
function snap(){
...
function askSharingThenTest(auto){
  /* Entry intent decides what follows the sharing answer. Arriving via #test
     (an invited tester) flows straight into task 1; arriving via the plain
     URL — including the landing page's "just try the keyboard" — gets an
     OFFER with a "later", because that link promised the keyboard, not a
     20-minute assignment. */
  const after=auto?()=>setTimeout(testStart,900):()=>setTimeout(testOffer,900);
  say("<b>မျှဝေမှု · sharing</b><br>"+
      "သင့်စာသားများ ဖုန်းထဲမှာပဲ ရှိပါမယ် · Your words stay on your phone. "+
      "I only send numbers.<br><br>"+
      "အဘိဓာန် ပိုကောင်းအောင် သင်ရိုက်သည်များကို မျှဝေမလား · "+
      "May I also send what you type, to improve the dictionary?"+
      (auto?"<br><br><span class='dim'>ပြီးရင် အလုပ်ငယ် ၁၈ ခု · then 18 short "+
      "tasks. Stop any time.</span>":""),[
    {label:"ဟုတ်ကဲ့ · yes",act:()=>{opt.oov=true;opt.donate=true;
      say("ကျေးဇူးပါ · thank you.");after()}},
    {label:"မသိသည့်စာလုံးများသာ · unknown words only",ghost:true,
      act:()=>{opt.oov=true;say("ရပါတယ် · noted.");after()}},
    {label:"မဟုတ်ပါ · numbers only",ghost:true,
...
function testStart(){
  assembleItems();                    // each session draws its own copy items
...
const FB_Q=[
 ...
 {k:"open",q:"နောက်ဆုံးတစ်ခု · one last thing.<br>"+
   "<b>ကြိုက်တာ မကြိုက်တာ ဘာမဆို ပြောခဲ့ပါ · anything at all — good, bad, "+
   "weird, missing?</b><br><span class='dim'>လွတ်လွတ်လပ်လပ် ရေးပါ · "+
   "say it however you like.</span>"},
];
function fbStart(){
...
function askQ(){
  ...
  if(!q)return fbFinish();     // two fast answers can race past the last question
...
function routeHash(){
  if((location.hash||"").slice(1)==="test"&&!convo.mode&&!msgs.length){
    /* an invited tester: sharing first if they have never answered it,
       then straight into task 1 */
    if(!stored(seenKey))setTimeout(()=>askSharingThenTest(true),500);
    else setTimeout(testStart,500);
  }
}
...
if(!stored(seenKey)&&location.hash.slice(1)!=="test")setTimeout(()=>askSharingThenTest(false),650);
</script></body></html>'''
